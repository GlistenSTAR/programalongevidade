...
class UserManagementController extends Controller
{
    ...
    public function updateCadastroPostAction(Request $poRequest)
    {
        ...
        if (PacienteModelConverter::requestToModel($poRequest, $loPaciente)) {
            ...
            $loResponse = $this->userManagementCAN->updateUser($loUserManagementRequest);
    ...
    public function cadastroAdesaoPostAction(Request $poRequest)
    {
        ...
        if (PacienteModelConverter::requestToModel($poRequest, $loPaciente) && MedicoModelConverter::requestToModel($poRequest, $loMedico)) {
            ...
            if ($loResponse->getisOperationSuccess() && (!empty($loResponse->getObject()) && !empty($loResponse->getObject()->LGV_USER_STATUS))) {
                ...
                if (!empty($loResponse->getObject())) {

                    $userToUpdate = $loResponse->getObject();
                    $sourceUser = $loUserManagementRequest->getPaciente();
...
//                    $userToUpdate->LGV_USER_DATA_NASCIMENTO = $sourceUser->LGV_USER_DATA_NASCIMENTO;
                    ...
                    $userToUpdate->LGV_USER_ENDERECO = $sourceUser->LGV_USER_ENDERECO;
                    $userToUpdate->LGV_USER_ENDERECO_NUMERO = $sourceUser->LGV_USER_ENDERECO_NUMERO;
                    $userToUpdate->LGV_USER_ENDERECO_COMPLEMENTO = $sourceUser->LGV_USER_ENDERECO_COMPLEMENTO;
                    $userToUpdate->LGV_USER_ENDERECO_BAIRRO = $sourceUser->LGV_USER_ENDERECO_BAIRRO;
                    $userToUpdate->LGV_USER_ENDERECO_CIDADE = $sourceUser->LGV_USER_ENDERECO_CIDADE;
                    $userToUpdate->LGV_USER_ENDERECO_ESTADO = $sourceUser->LGV_USER_ENDERECO_ESTADO;
                    $userToUpdate->LGV_USER_ENDERECO_CEP = $sourceUser->LGV_USER_ENDERECO_CEP;
                    $userToUpdate->LGV_USER_TELEFONE_FIXO_DDD = $sourceUser->LGV_USER_TELEFONE_FIXO_DDD;
                    $userToUpdate->LGV_USER_TELEFONE_FIXO_NUMERO = $sourceUser->LGV_USER_TELEFONE_FIXO_NUMERO;
                    $userToUpdate->LGV_USER_TELEFONE_CELULAR_DDD = $sourceUser->LGV_USER_TELEFONE_CELULAR_DDD;
                    $userToUpdate->LGV_USER_TELEFONE_CELULAR_NUMERO = $sourceUser->LGV_USER_TELEFONE_CELULAR_NUMERO;
                    $userToUpdate->LGV_USER_DATA_CADASTRO = $sourceUser->LGV_USER_DATA_CADASTRO;
                    $userToUpdate->LGV_USER_DATA_ULTIMA_ALTERACAO = $sourceUser->LGV_USER_DATA_ULTIMA_ALTERACAO;
                    $userToUpdate->LGV_USER_STATUS = $sourceUser-> LGV_USER_STATUS;
                    $userToUpdate->LGV_USER_CPF = $sourceUser->LGV_USER_CPF;
                    $userToUpdate->LGV_USER_LGPD = $sourceUser-> LGV_USER_LGPD;
                    $userToUpdate->LGV_USER_LGPD_DATA = Carbon::now("America/Sao_Paulo");

                    $loUserManagementRequest->setPaciente($userToUpdate);
    ...
    public function AcceptUserTerms(Request $request) {
        ...
        if ($aceite->AceiteTermo(new AceiteTermo($loSessaoPDV->LGV_CODIGO, $lsDate,69, $lsDate, 66, 0, 83, intval($loPaciente->LGV_USER_CPF), intval($loPaciente->LGV_USER_TELEFONE_CELULAR_DDD), intval($loPaciente->LGV_USER_TELEFONE_CELULAR_NUMERO), Config::get("app.SEVEN_SEU_SISTEMA"), $loSessaoPDV->LGV_SESSAO, Config::get("app.SEVEN_CTL_AP"), Config::get("app.SEVEN_DEFAULT_ADMINISTRADORA"), "abcdefg", "ADES_URA", "01", "Site", "form-012345678", $request->ip()))) {
            ...
            if (($aceite->getResult()->getAceiteTermoResult()->getStatusServico() >= 40 && $aceite->getResult()->getAceiteTermoResult()->getStatusServico() <= 49) ||  $aceite->getResult()->getAceiteTermoResult()->getRetCode() == "Q076" ||  $aceite->getResult()->getAceiteTermoResult()->getRetCode() == "Q022") {
                ...
                $response = $this->userManagementCAN->updateUser($request);
